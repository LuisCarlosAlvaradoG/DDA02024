...
def problem_01(arr):
    ...
    m = a.mean()
    s = a.std()   # desviación estándar poblacional
    if s == 0:
        res = np.zeros_like(a)
    else:
        res = (a - m) / s
...
for inp, exp in zip(_np_in, _np_exp):
    ...
    if np.all(res == exp):
        correct += 1
print(f"Problema 01 Parcial 03 V3: {correct}/10")
...
for data, exp in zip(_pd_in, _pd_exp):
    ...
    if isinstance(res, pd.DataFrame):
        ...
        if res.equals(exp_df):
            correct += 1
print(f"Problema 02 Parcial 03 V3: {correct}/10")
...
for mat, thr, exp in zip(_comb_mat, _comb_thr, _comb_exp):
    ...
    if res == exp:
        correct += 1
print(f"Problema 03 Parcial 03 V3: {correct}/10")
